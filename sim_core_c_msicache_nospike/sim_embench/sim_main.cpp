...
#define PROGRAM_START_ADDR 0x10074 //the start addr of the program, 0x00000000 default
#define EXE_BEGIN_ADDR     0x100b0 //0x10070 // should align to a line (8 bytes)
...
int bin2cm(char source[], uint32_t *mem);
...
{
    strcpy(source[loop_i], argv[1]/*"../embench.matmult-int.bin"*/);
...
//uint64_t* mem0 =NULL;
//uint64_t* mem1 =NULL;
uint32_t* mem[CORE_NUM_MAX];
for(loop_i=0;loop_i<numP;loop_i++)
{
    mem[loop_i] = (uint32_t *)malloc(size[loop_i]);
    memset(mem[loop_i], 0, size[loop_i]);
...
/*  unsigned int memory_PROGRAM_start_addr_load  = 0x100000/(8*sizeof(unsigned int));
    unsigned int memory_PROGRAM_start_addr_store = 0x300000/(8*sizeof(unsigned int));
   for (count = memory_PROGRAM_start_addr_load; count < memory_PROGRAM_start_addr_store; count++)
    {
        for(count2 = 0; count2 < 8; count2++)
        {
    ...
    */
    ...
    unsigned int memory_PROGRAM_start_addr_load  = 0;
    unsigned int memory_PROGRAM_start_addr_store = 0x11440 / 32;
  /*  for (count = memory_PROGRAM_start_addr_load; count < memory_PROGRAM_start_addr_store; count++)
    {
        for(count2 = 0; count2 < 8; count2++)
        {
...
*/
  ...
  */
     for(loop_i=0;loop_i<numP;loop_i++)
     {
        int blank_before_program = PROGRAM_START_ADDR / sizeof(uint64_t);
        int inst_mem_count       = blank_before_program + size[loop_i]/sizeof(uint64_t) + 1;
        int zero_divide          = blank_before_program;
        for (count = 0; count < inst_mem_count; count++) 
        {
          if(count < zero_divide)
          {
              top[loop_i]->inst_mem_i[count] = 0;
          }
          else if(count == zero_divide)
          {
              top[loop_i]->inst_mem_i[count] = 0 + ((uint64_t)mem[loop_i][(count-zero_divide)*2] << 32);
          }
          else
          {
              top[loop_i]->inst_mem_i[count] = (uint64_t)mem[loop_i][(count-zero_divide)*2-1] + ((uint64_t)mem[loop_i][(count-zero_divide)*2] << 32);
          }
        

          printf("inst addr 0x%08x  0x%016lx\n",count*8, top[loop_i]->inst_mem_i[count]);
        }
     }
    ...
    for (count = memory_PROGRAM_start_addr_load; count < memory_PROGRAM_start_addr_store; count++)
    {
        for(count2 = 0; count2 < 8; count2++)
        {
            memory[count][count2] = (top[0]->inst_mem_i[count*4+count2/2] >> (32*(count2%2)))%0x100000000;
            if(memory[count][count2]!=0)
            {
                printf("memory[%d][%d]=0x%x \n", count, count2, memory[count][count2]);
            }
...
for(loop_m = start_cyc; loop_m < numP; loop_m++)
{
    ...
    //for(loop_m = 0; loop_m < run_cycle; loop_m++)
    {
        ...
        { 
            top[loop_i]->rst_addr_i = EXE_BEGIN_ADDR;
