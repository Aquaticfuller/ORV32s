...
{
    strcpy(source[loop_i], argv[1] /*"../dhrystone.bin"*/);
...
/*  unsigned int memory_start_addr_load  = 0x100000/(8*sizeof(unsigned int));
  ...
  unsigned long int mcycle_last[4] = {0};
...
for(loop_m = start_cyc; loop_m < numP; loop_m++)
{
    ...
    {
        char reverse_en;
        reverse_en = main_time % CORE_NUM_MAX;
        if(reverse_en == 0 | reverse_en == 1) //reverse exe
        {
            loop_i = numP - loop_m - 1;
//printf("core   %d       reverse   1\n",loop_i);
        }
        else
        {
            loop_i = loop_m;
//printf("core   %d       normal    0\n",loop_i);
        }
    }
//printf("core   %d\n",loop_i);
...
*/
      ...
      if(0/*top[loop_i]->dff_ireg_ver[6] == 100*/) 
      {
          if(prosFin_en[loop_i] == 0)
          {
...
//printf("state change to %d %d\n",top[loop_i]->cacheLineValid_i[top[loop_i]->reqIndex_o],top[loop_i]->cacheLineValid_o[top[loop_i]->reqIndex_o]);
          ...
          /******************print tohost******************/
               if(top[loop_i]->data_addr_o == 0x1000 && top[loop_i]->clk == 0 && mcycle_last[loop_i] != top[loop_i]->dff_csr_mcycle_ver)
               {
                   mcycle_last[loop_i] = top[loop_i]->dff_csr_mcycle_ver;
                 ...
                 //  printf("bufadd = 0x08%x\n",stackmemadd);
                  // printf("len = %d\n", buflen);
                  // printf("pc = 0x%x\n", top[loop_i]->pc_stage2_ver);
                  // printf("mcycle = %ld\n",  top[loop_i]->dff_csr_mcycle_ver);
                   if(buflen < 50)
                   {
                       for(int i = 0; i < buflen/2 + buflen%2; i++ )
                       {
                           printf("%.4s", (top[loop_i]->cacheLine_o[stackmemadd_index]+i) );
                       }
                   }
                   
                   //printf("%4.4s",top[loop_i]->cacheLine_o[stackmemadd_index]); 
