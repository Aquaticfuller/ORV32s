...
{return 0;}
int spikeaaa(int argc, char** argv, int step_num,  std::vector<state_t> &reg_record, std::vector<insn_t> &inst_record)
{
  bool debug = false;
  bool halted = false;
 ...
 *--isa,
  ...
  reg_record  = s.log_XPR;
  inst_record = s.log_INST;
                                                                                        fprintf(stderr, "Position 6 \n");
                                                                                        std::cout<<"return code = "<<return_code<<std::endl;

/*  for (auto it = s.log_XPR.begin() ; it != s.log_XPR.end(); ++it)
  {
	  //int r =1;
...
/*  processor_t *p = s.get_core(0);
  ...
  fprintf(stderr, "Position 200 \n");*/



 	for (auto& mem : mems)
    delete mem.second;
 	// fprintf(stderr, "Position 201 \n");
  for (auto& plugin_device : plugin_devices)
    delete plugin_device.second;
  //fprintf(stderr, "Position 202 \n");
  delete command;
  //fprintf(stderr, "Position 203 \n");
  return return_code;
}
