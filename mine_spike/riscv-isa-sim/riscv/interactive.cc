...
void sim_t::interactive()
{
  ...
  while (!done())
  {
	...
	//	      //template <class T, size_t N, bool zero_reg> class regfile_t
	...
	//	    }
		...
		std::string cmd, tmp;   // cmd 保存 命令, 如r // tmp 保存对应参数, 如1
...
void sim_t::interactive_run(const std::string& cmd, const std::vector<std::string>& args, bool noisy)
{
  ...
  {
    step(1);
                                                                  // fprintf(stderr, "Position interactive_run, store\n");

    processor_t *p = get_core(0);            // record reg log
    //log_XPR.clear();
    log_XPR.push_back(*(p->get_state()));

    std::vector<std::string> argu;
    argu.push_back("0");
    reg_t pc = get_pc(argu);
    insn_fetch_t fetch = (p->get_mmu())->load_insn(pc);
    insn_t inst = fetch.insn;
    log_INST.push_back(inst);
    // disasm(fetch.insn);                 //  disasm and print
  }
}
