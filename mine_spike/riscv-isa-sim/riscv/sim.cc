...
void sim_t::main()
{
 ...
 // step(step_num);
  while (!done())
  {
    ...
    {
    	             fprintf(stderr, "Position 20\n");
      //interactive(log_XPR,log_switch);               // interactive debug
      interactive();
                     fprintf(stderr, "Position 21\n");
    }
    else
    	step(INTERLEAVE);
    	//step(step_num);
...
//    if(step_num <= 0)
//    	break;
                            fprintf(stderr, "Position 22\n");
  }
                                 fprintf(stderr, "Position 23\n");
...
int sim_t::run()
{
  host = context_t::current();        //  context_t:  fesvr/context.h
  //htif_t::restart();
  fprintf(stderr, "Position before\n");
  target.init(sim_thread_main, this);     // -> sim_thread_main-> sim_t::main() -> interactive();
  fprintf(stderr, "Position after\n");
  //return 0;

  return  htif_t::run();               //  htif_t:     fesvr/htif.h
  //delete host;
  //return retur;
}

void sim_t::step(size_t n)   // n = how many steps to go
{
  for (size_t i = 0, steps = 0; i < n; i += steps)
  {
    steps = std::min(n - i, INTERLEAVE - current_step);
    procs[current_proc]->step(steps);             // std::vector<processor_t*> procs; //  riscv/processor.h
...
void sim_t::idle()
{
	fprintf(stderr, "Position 32 \n");
  target.switch_to();
  fprintf(stderr, "Position 33 \n");
}
