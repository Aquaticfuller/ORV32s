...
#ifdef USE_UCONTEXT                                  //defined
                                                                      fprintf(stderr, "Position 11\n");
  getcontext(context.get());
  context->uc_link = creator->context.get();    // context运行完了之后, 会到指向creator创建出的context继续运行
  context->uc_stack.ss_size = 64*1024;          // 设置新的上下文堆栈大小
  context->uc_stack.ss_sp = new void*[context->uc_stack.ss_size/sizeof(void*)]; // 创建上下文堆栈
#ifndef GLIBC_64BIT_PTR_BUG              //not defined
                                                                       fprintf(stderr, "Position 12\n");
  makecontext(context.get(), (void(*)(void))&context_t::wrapper, 1, this);
#else
                                                                          fprintf(stderr, "Position 13\n");
...
void context_t::switch_to()
{
  ...
  if (swapcontext(prev->context.get(), context.get()) != 0){
	  fprintf(stderr, "Position 40-1 \n");
	  abort();
  }fprintf(stderr, "Position 40-2 \n");
#else
  cur->flag = 0;
  this->flag = 1;
