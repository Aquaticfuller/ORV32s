...
void htif_t::load_program()
{
  ...
  std::map<std::string, uint64_t> symbols = load_elf(path.c_str(), &preload_aware_memif, &entry);
         // std::map<std::string, uint64_t> load_elf(const char* fn, memif_t* memif, reg_t* entry) // fesvr/elfloader.cc

  if (symbols.count("tohost") && symbols.count("fromhost")) {
    tohost_addr = symbols["tohost"];          //uint64_t
...
int htif_t::run()
{
  start();       // here we get load_program(), where load 'pk'
  ...
  if (tohost_addr == 0) {
    while (true)
      idle();
                                                                    ...
                                                                    std::cout<<"tohost_addr = "<<tohost_addr<<std::endl<<"fromhost_addr = "<<fromhost_addr<<std::endl;
  while (!signal_exit && exitcode == 0)
  {
	                                                                          fprintf(stderr, "Position 7 \n");
	                                                                //std::cout<<"tohost_addr = "<<tohost_addr<<"  "<<"fromhost_addr = "<<fromhost_addr<<std::endl;
    //if (auto tohost = from_le(mem.read_uint64(tohost_addr))) { // 读给定内存, 并进行endian转换
	auto tohost = from_le(mem.read_uint64(tohost_addr));
	                                                     fprintf(stderr, "Position 31 tohost = %ld \n" ,tohost);
	if (tohost) {
        // memif_t mem;  //  fesvr/memif.h
        // from_le   /riscv/byteorder.h  from little endian
                        fprintf(stderr, "Position 7-1 \n");
      mem.write_uint64(tohost_addr, 0);  //  fesvr/memif.h
      command_t cmd(mem, tohost, fromhost_callback);  // fesvr/device.h
      device_list.handle_command(cmd);                                 // std::cout<<"cmd = "<<cmd<<std::endl;
    } else {
    	fprintf(stderr, "Position 7-2 \n");
      idle();
      fprintf(stderr, "Position 7-3 \n");
      break;
    }
                                                                                  fprintf(stderr, "Position 8 \n");
    device_list.tick();

    // write fromhost to tell pk the syscall is in the call queue, pk can go on
